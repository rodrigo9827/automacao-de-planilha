...
echo [i] - Verificando diretorio de origem...
if not exist "%userprofile%\Desktop\atendimento_rodada" (
    mkdir "%userprofile%\Desktop\atendimento_rodada"
)

echo [i] - Movendo arquivos TXT para a pasta do executavel...
move "%userprofile%\Desktop\atendimento_rodada\*.txt" "%userprofile%\Downloads\automacao-de-planilha-main\automacao-de-planilha-main\executavel\teste_let" >nul 2>&1

echo [+] - Entrando na pasta do script...
cd /d "%userprofile%\Downloads\automacao-de-planilha-main\automacao-de-planilha-main\executavel\teste_let"

echo [+] - Iniciando o script Python...
:: IMPORTANTE: Substitua 'seu_script.py' abaixo pelo nome do seu arquivo Python (ex: main.py)
python juntando_boot_GUI_let-test1.py
